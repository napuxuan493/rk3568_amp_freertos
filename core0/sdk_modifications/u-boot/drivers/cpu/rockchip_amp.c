...
int amp_cpus_on(void)
{
	...
	/*
	 * M2.1 fix: read a bounded prefix of the amp partition in one go.
	 *
	 * The original code only read FIT_HEADER_SIZE (4 KiB) first, then
	 * looked up the root "totalsize" property with fdt_getprop(). The
	 * property NAME lives in the FDT strings block (off_dt_strings),
	 * which sits BEYOND the first 4 KiB for any FIT larger than ~4 KiB
	 * (e.g. a firmware payload > ~2.5 KiB with inline data). The name
	 * lookup then read out of bounds, fdt_getprop() returned NULL and
	 * amp_cpus_on() bailed with "AMP Error: No totalsize".
	 *
	 * Reading a bounded prefix (4 MiB) keeps the strings block always
	 * available while staying below the dwmmc 16-bit block-count limit
	 * (0x10000 blocks = 32 MiB for this 32 MiB partition).
	 */
	#define AMP_READ_BLOCKS		(SZ_4M / 512)
	hdr = memalign(ARCH_DMA_MINALIGN, SZ_4M);
	if (!hdr) {
		printf("AMP: no memory for amp partition buffer\n");
		return -ENOMEM;
	}

	/* get totalsize (bounded prefix already in hdr) */
	...
	if (totalsize > cnt * (int)part.blksz) {
		AMP_E("FIT too large: %d > %d\n", totalsize, cnt * (int)part.blksz);
		ret = -EINVAL;
		goto out2;
	}

	/* prefix already loaded: reuse hdr as the fit buffer */
	fit = hdr;
	hdr = NULL;
	cnt = 0;
